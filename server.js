...
const SESSIONS_FILE = '/tmp/egh_sessions.json';
const VALID_SESSIONS = new Map();

// Load sessions from disk on startup
try {
  const saved = JSON.parse(require('fs').readFileSync(SESSIONS_FILE,'utf8'));
  Object.entries(saved).forEach(([k,v]) => VALID_SESSIONS.set(k,v));
  console.log('✓ Loaded', VALID_SESSIONS.size, 'sessions from disk');
} catch(e) { /* no saved sessions yet */ }

function saveSessions(){
  try {
    const obj = {};
    VALID_SESSIONS.forEach((v,k) => obj[k]=v);
    require('fs').writeFileSync(SESSIONS_FILE, JSON.stringify(obj));
  } catch(e) {}
}
...
async function initMinified() {
  ...
  console.log('✓ HTML loaded and cached');
}
...
app.post('/api/verify-key', async (req,res) => {
  ...
  try {
    ...
    // KeyAuth requires hwid field even when Force HWID is off
    // We generate one from the key itself so it's consistent per key
    const hwid = crypto.createHash('sha256').update(key + 'epichub').digest('hex').slice(0, 32);
    ...
    if(licData.success) {
      ...
      VALID_SESSIONS.set(token, { discord, key, expires: Date.now() + 24*60*60*1000 }); saveSessions();
      try {
...
const COSMETICS = [
  ...
  { id:'minty',     type:'cosmetic', name:"Minty Axe",          price:"1,000.00", color:"#0fc864", img:"https://images.g2a.com/470x276/1x1x0/fortnite-minty-pickaxe-skin-epic-games-key-europe-i10000192306002/5def65df7e696c1c6f40a132", desc:"A legendary Harvesting Tool that combines a tough design with a minty fresh touch." },
  ...
  { id:'wildcat',   type:'cosmetic', name:"Wildcat",            price:"45.00",    color:"#e85d04", img:"https://imgproxy.eneba.games/V71C-gUhQNVfeMzXzaAeKCieFaEursCj7ckTzy4qvbs/rs:fit:1920/ar:1/czM6Ly9wcm9kdWN0/cy5lbmViYS5nYW1l/cy9wcm9kdWN0cy83/T2J4QVA3Wk9NMlJI/MVBtak1kc1hVQTNm/RjhtdjNVLU8yQ1Nj/c1BtLTZVLmpwZWc", desc:"The fierce Wildcat Outfit — a Nintendo Switch exclusive bundle skin with a bold orange and black style." },
  { id:'wonder',    type:'cosmetic', name:"Wonder",             price:"55.00",    color:"#6a0572", img:"https://i.redd.it/w59td673imdc1.jpeg", desc:"The ultra-rare Wonder Outfit, one of Fortnite's most sought-after exclusive cosmetics." },
  { id:'renegade',  type:'cosmetic', name:"OG Renegade Raider", price:"500.00",   color:"#8b1a1a", img:"https://i.redd.it/the-og-styles-for-renegade-raider-and-aerial-assault-v0-iuwz6sd233de1.jpg?width=1080&format=pjpg&auto=webp&s=808aa485fb8c2772cdc3860ef40f2c046375af6e", desc:"The rarest skin in Fortnite history. Only available to Season 1 players who reached Battle Pass Level 20." },
];
...
// ══════════════════════════════════════════════════════════════
// DEMO ENDPOINT — replace with a real player-lookup API
// A real integration would call the actual game's player/friends API
// here (e.g. verify the username exists, fetch their real avatar/level)
// instead of generating placeholder data.
// ══════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════
// DEMO ENDPOINT — replace with a real payment provider integration
// A real integration would create a real charge/order here via your
// payment processor's server-side SDK (e.g. PayPal Orders API,
// Stripe PaymentIntents, etc.), verify the result server-side, and
// only then return the confirmed transaction details below — never
// trust client-supplied price/item data for a real charge.
// ══════════════════════════════════════════════════════════════
